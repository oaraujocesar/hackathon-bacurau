  ...
  <div class="main">
    ...
    <div class="todo">
      <div class="board">
        <h4>Curto prazo</h4>
        <div class="task-list">
          <div v-for="{id, taskname, estimative, concluded} in sanitazedSubgoals" :key="id">
          <Subgoal v-if="estimative === 'curto'" :id="id" :text="taskname"  :isConcluded="concluded" @finish="(id) => checkHandler(id)"/>
          </div>
        </div>
      </div>
      <div class="board">
        ...
        <div class="task-list">
        <div v-for="{id, taskname, estimative, concluded} in sanitazedSubgoals" :key="id">
          <Subgoal v-if="estimative === 'medio'" :id="id" :text="taskname"  :isConcluded="concluded" @finish="(id) => checkHandler(id)"/>
          </div>
        </div>
      </div>
      <div class="board">
        ...
        <div class="task-list">
        <div v-for="{id, taskname, estimative, concluded} in sanitazedSubgoals" :key="id">
          <Subgoal v-if="estimative === 'longo'" :id="id" :text="taskname" :isConcluded="concluded" @finish="(id) => checkHandler(id)"/>
        </div>
      </div>
    </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    checkHandler(id){
      let position
      const subgoalObj = this.subGoals.find((item, index) => {
        position = index
        return item.id === id ? item : null
      })
      this.subGoals.splice(position, 1, {
        id: subgoalObj.id,
        estimative: subgoalObj.estimative,
  ...
  computed: {
    ...
    sanitazedSubgoals(){
      if(this.subGoals.length){
        return this.subGoals.sort((a,b) => a.concluded > b.concluded)
      }
    }
  }
}
